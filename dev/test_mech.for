      include '../UEL-Mech/src/uel_mech.for'

! **********************************************************************
! **********************************************************************

!     using ifort compiler compile the code from intel oneAPI terminal:
!     ifort /Qmkl -o mech test_mech.for
!     this command will generate an executable call mech
!     execute the pegel on the terminal using: .\mech
...
! **********************************************************************
      ...
      if (case.eq.1) then
        ...
        fileName = 'mechDebug_1.dat'
      else if (case.eq.2) THEN
        write(*,*) 'testing case-II'
        fileName = 'mechDebug_2.dat'
        lam_1_App = two
      else
        write(*,*) 'wrong case'
        stop
      ...
      JTYPE   = 7         ! type of element (U3: HEX8, U7: QUAD4-PE, U15: QUAD4-AX)
      ...
      NPROPS  = 2               ! total no of material properties
      MCRD    = nDim            ! no of dimension (ABAQUS does it differently)
      NJPROPS = 2               ! no of integer properties
     ...
     &      PREDEF(2,NPREDF,1), DDLMAG(NNODE,1), JPROPS(NJPROPS))
      ...
      LFLAGS  = [1, 0, 0, 0, 0]   ! Abaqus step flag
      VEL     = zero              ! velocity
      ...
      PROPS(1:NPROPS)   = [ E, nu ]
      JPROPS(1:NJPROPS) = [ nInt, nPostVars ]
     ...
     &      NJPROPS,PERIOD)
      ...
      do i = 1, NDOFEL
        write(fileUnit, '(*(F24.8))') AMATRX(i,:)
      end do


       write(fileUnit,'(A)') 'AMATRX_h: '
      do i = 1, NDOFEL
        write(fileUnit, '(*(F24.8))') AMATRX_h(i,:)
      end do
